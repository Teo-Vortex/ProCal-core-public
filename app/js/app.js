...
renderMainPanelUI();
  ...
  setText("filesCapacityLabel", t("filesCapacityFormat").replace("{used}", "0 MB").replace("{limit}", t("filesUnlimited")));
...
function renderFilesCapacity(usedBytes, limitBytes) {
  ...
  if (filesCapacityLabel) {
    ...
    const limitText = limit > 0 ? formatBytes(limit) : t("filesUnlimited");
